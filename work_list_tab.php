...
//echo $prev." -- ".$next;exit('Priyesh');
    ...
    {   
        ...
        //previous button
        if ($page > 1) 
            $pagination.="<a class=\"btn btn-sm btn-warning\" type=\"button\" data-toggle=\"tooltip\" data-original-title=\"Edit this user\" href=\"work_list.php?page=$prev\"><i>« Previous</i></a>";
            //$pagination.= "<a href=\"user_list.php?page=$prev\">« previous</a>";
        else
            $pagination.="<a class=\"disabled btn btn-sm btn-warning\" type=\"button\" data-toggle=\"tooltip\" data-original-title=\"Edit this user\" href=\"work_list.php?page=$prev\"><i>« Previous</i></a>";
           // $pagination.= "<span class=\"disabled\">« previous</span>"; 
            $pagination.="&nbsp &nbsp &nbsp";
        //next button
        if ($page < $lastpage) 
            $pagination.= "<a class=\"btn btn-sm btn-danger\" type=\"button\" data-toggle=\"tooltip\" data-original-title=\"Remove this user\" href=\"work_list.php?page=$next\"><i>next »</i></a>";
            //$pagination.= "<a href=\"user_list.php?page=$next\">next »</a>";
        else
            $pagination.= "<a class=\"disabled btn btn-sm btn-danger\" type=\"button\" data-toggle=\"tooltip\" data-original-title=\"Remove this user\" href=\"work_list.php?page=$next\"><i>next »</i></a>";
           // $pagination.= "<span class=\"disabled\">next »</span>";
       // $pagination.= "</div>\n";       
    }

$record = $renderObj->get_work_list_pagination($_SESSION['userinfo']->id,$start,$limit);
$review_outbox = $renderObj->get_review_outbox($_SESSION['userinfo']->id);
//echo "<pre>";print_r($review_outbox);exit;
...
<div class="panel-body">
  ...
  <ul class="nav nav-tabs" role="tablist">
    <li role="presentation" class="active"><a href="#home" aria-controls="home" role="tab" data-toggle="tab">Review Inbox</a></li>
  ...
  <div class="tab-content">
	  ...
	  <?php 
            $flag = 0;
            //echo "<pre>";print_r($record);die;
            if(!empty($review_outbox)){
            foreach ($review_outbox as $key=>$val) { 
	    $request_status = $renderObj->get_request_status($val['id']);
            //echo "<pre>";print_r($request_status);
            //$status = ($request_status['status'] != '' ? $request_status['status'] : 'N/A');
              if (array_key_exists($request_status['status'],$status_flag)) {
	  ...
	  ?>
          ...
          <tr>
            <td><?php echo $newDate = date("d-M-Y", strtotime($val['created_date']));?></td>
