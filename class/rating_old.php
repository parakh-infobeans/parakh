...
class rating {
    ...
    function is_profile_access_allowed($user_id, $lead_id) {
        $dbh = $this->get_connection();
        if ($dbh) {
            $query = "SELECT DISTINCT uh.user_id, "
                    . "uh.manager_id FROM " . self::TAB_USER_HIERARCHY . " "
                    . "as uh JOIN " . self::TAB_USER . " u ON uh.user_id=u.id "
                    . "WHERE   u.status =1 AND  (u.id=:id) AND (uh.manager_id=:lead_id)";
    ...
    function save_emp_rating($data) {
        ...
        if ($dbh) {
            ...
            if (empty($rating_inst)) {
                ...
                $rate = $data['rating'];
                $sql = "Update " . self::TAB_RATING . " SET " . self::TAB_RATING_SHOW_RATING . " ='" . $show . "',rating = $rate,modified_date = '" . date('Y-m-d H:m:s') . "' WHERE request_id = '" . $data['request_id'] . "'";
                $query = $dbh->prepare($sql);
                $data2 = $query->execute();
            }
            ...
            $sql = "Update " . self::TAB_REQUEST . " SET show_request ='" . $show . "',status = '" . $status . "', modified_date = '" . date('Y-m-d H:m:s') . "' WHERE id = '" . $data['request_id'] . "'";
            $query = $dbh->prepare($sql);
            $data2 = $query->execute();
            return true;
        }
    ...
    function decline($data) {
        ...
        if ($dbh) {
            ...
            $created_date = $modified_date = $dateTime->format("Y-m-d H:i:s");
            $id = $data['request_id'];
            $sql = "Update " . self::TAB_REQUEST . " SET show_request='1',status = '1', modified_date = '" . date('Y-m-d H:m:s') . "' WHERE id = '" . $id . "'";
            $query = $dbh->prepare($sql);
            $data2 = $query->execute();
    ...
    function get_work_rating($user_id) {
        $dbh = $this->get_connection();
        if ($dbh) {
            $query = "SELECT work.work_date,work.title,if( comment.comment_text != '', comment.comment_text, work.description ) AS description,work.created_by as given_by,user.google_name as given_by_name,rating.user_id,rating.rating as rating,rating.id as rate_id,work.created_date, comment.comment_text AS manager_comment"
                    ...
                    WHERE rating.user_id= :user_id  ORDER BY work.id desc ";
            $user_list = $dbh->prepare($query);
    ...
    function get_user_request_details($user_id = null, $status = '') {
        ...
        if ($dbh) {
            if (isset($user_id)) {
                ...
                $query = "select user.google_name, user.google_picture_link, role.name as role_name, request.to_id,request.from_id,description,work.created_date,request_for,rating, request.status from work as work left join request as request on work.id = request.work_id left join rating as rating on work.id=rating.work_id left join  users as user on request.to_id=user.id left join role_type as role on role.id = user.role_id where work.created_by = " . $user_id . $cnd . " order by work.id desc";
                $user_list = $dbh->prepare($query);
                $user_list->execute();
                $row = $user_list->fetchAll((PDO::FETCH_ASSOC));
                return $row;
