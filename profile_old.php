...
$record = $renderObj->get_profile($user_id);// Put session id here
$get_work_rating = $renderObj->get_work_rating($user_id);
//echo"<pre>";
//print_r($get_work_rating);
//echo "</pre>";
  if($record != 0) {
...
$html_for_minus_one = $html_for_plus_one = '';
	...
	$plus_rating++;
        $html_for_plus_one .=                                                 
            '<div class="profile-lft-row"><div class="pforile-lft-row-col"  style="padding-right:5px">';
       
        if(!empty($_REQUEST['edit_comment']) && ($val['given_by']==$_SESSION['userinfo']->id) 
                        && ($val['user_id']!=$_SESSION['userinfo']->id))
                {
               ...
               $html_for_plus_one .='<div class="pforile-lft-row-col2">'.$val['given_by_name'].'&nbsp;</div>
                <div class="pforile-lft-row-col2">'.$date.'&nbsp;</div>';
	...
	$minus_rating++;
         $html_for_minus_one .=                                                 
            '<div class="profile-lft-row">
                <div class="pforile-lft-row-col" style="padding-right:5px">';
                if(!empty($_REQUEST['edit_comment']) && ($val['given_by']==$_SESSION['userinfo']->id) 
                               && ($val['user_id']!=$_SESSION['userinfo']->id))
                       {
                           $val['description']=($val['description']=="N/A")? "": $val['description'];
                ...
                $html_for_minus_one .= '<div class="pforile-lft-row-col2">'.$val['given_by_name'].'&nbsp;</div>
                <div class="pforile-lft-row-col2">'.$date.'&nbsp;</div>
            </div>';                                          
        }                                 
    } 
    
    if($html_for_minus_one == ''){
        $html_for_minus_one = "<div class='center'>No ratings available.</div>";
    }
    if($html_for_plus_one == ''){
        $html_for_plus_one = "<div class='center'>No ratings available.</div>";
    }    

if(isset($chk) && $chk>0){ ?>			
	<div class="succes-green"><div class="close-alert"></div> <?php echo $errors[$chk]; ?></div>
<?php } ?>
   <section>
        ...
        <div class="wrapper">
            <div class="mid-col-12">
                <div class="profile-wrapper">
                    ...
                    <?php if(empty($_REQUEST['edit_comment']))
                    { ?>
                    <a class="link-txt-request" style="margin-top:-50px" href="edit_profile.php">Edit Profile </a>
                    <?php } ?>
				   ...
				   <?php if($plus_rating > 0){ echo "+".$plus_rating; } else { echo "0"; } ?>
				    
				   </span>
                            </div>
                            <?php echo $html_for_plus_one; ?>
                        </div>
                        <div class="profile-rht-report">
                            <div class="profile-lft-row">
                                   <span class="request-negative-red-3">
				   <?php if($minus_rating > 0){ echo "-".$minus_rating; } else { echo "0"; } ?>
				</span>
                           </div>
                           <?php echo $html_for_minus_one; ?>
                        </div>                         
                    </div>
...
<style>
   ...
   .editable-click, a.editable-click {
    ...
    header{
            background:#3c3c3c;
            /*background:-linear-gredient  background: -webkit-linear-gradient(#232424, #010101); 
            background: -o-linear-gradient(#232424, #010101); 
            background: -moz-linear-gradient(#232424, #010101);
            background: linear-gradient(#232424, #010101);*/
            height:111px;
    }
    footer{	
            background:#adadad;
